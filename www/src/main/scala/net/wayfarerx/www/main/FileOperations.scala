...
/**
 * Trait that supplies the IO-aware path/file/directory model of the filesystem.
 */
trait FileOperations {
  ...
  sealed trait Resource {
    ...
    /** The factory for this kind of resource. */
    def factory: Resource.Factory[This]
    ...
    final def rebase(from: Directory, to: Directory): IO[This] =
      ...
      else IO.pure(factory.from(to.path.resolve(path.subpath(from.path.getNameCount, path.getNameCount))))
  ...
  object Resource {
     ...
     *
     * @tparam T The type of resource this factory creates.
     */
    sealed trait Factory[T <: Resource] {
   ...
   */
  case class File private[FileOperations](path: Path) extends Resource {

    /* Set the type of this resource. */
    override type This = File

    /* Provide the file factory. */
    override def factory: Resource.Factory[This] = File
   ...
   * Factory for files.
   */
  object File extends Resource.Factory[File] {

    /* Provide file instances. */
    override def from(path: Path): File = File(path)
    ...
    def assume(path: Path): IO[File] = for {
      fileOpt <- IO(if (!Files.exists(path) || Files.isRegularFile(path)) Some(from(path)) else None)
    ...
    def attempt(path: Path): IO[Option[File]] = IO {
      if (Files.isRegularFile(path)) Some(from(path))
      else None
    }
   ...
   */
  case class Directory private[FileOperations](path: Path) extends Resource {

    /* Set the type of this resource. */
    override type This = Directory

    /* Provide the file factory. */
    override def factory: Resource.Factory[This] = Directory
   ...
   */
  object Directory extends Resource.Factory[Directory] {

    /* Provide directory instances. */
    override def from(path: Path): Directory = Directory(path)
    ...
    def assume(path: Path): IO[Directory] = for {
      directoryOpt <- IO(if (!Files.exists(path) || Files.isDirectory(path)) Some(from(path)) else None)
    ...
    def attempt(path: Path): IO[Option[Directory]] = IO {
      if (Files.isDirectory(path)) Some(from(path))
      else None
    }
